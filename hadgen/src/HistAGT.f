...
C
      IF(IPR.eq.1)WRITE(36,10)NEVENT                        ! HISTAGT 
   10 FORMAT(/ 20X,'COMPLETED ===STARAGT=== No.',I3)        ! HISTAGT   
      IF(IPR.eq.1)WRITE(36,20)                              ! HISTAGT
...
C         Conservation laws
        ...
        END IF
C
        IF(IPR.eq.1)WRITE(36,30)J,PX,PY,PZ,E,PARTIC(IPART)  ! HISTAGT
   30     FORMAT(I3,1X,3E11.4,E13.4,2X,A7)            
  100 CONTINUE
C -------------------- end of SPT loop -----------------
C
  200 CONTINUE
      IF(IPR.eq.1)WRITE(36,39)                              ! HISTAGT
   39 FORMAT(66('-'))
      IF(IPR.eq.1)WRITE(36,40)PXSUM,PYSUM,PZSUM,ESUM,RMSUM, ! HISTAGT
...
C RESIDUAL NUCLEI CHARACTERISTICS
      IF(IPR.eq.1)WRITE(36,50)                              ! HISTAGT
   50 FORMAT('     Residual nuclei:',34X,'A*   Z*    U*(MeV)')
C
      IF(IPR.eq.1)WRITE(36,61)1,SNU(1,1),SNU(2,1)           ! HISTAGT
...
C
      DO 80 J=2,LS11                       ! Nuclei-products
        ...
        IF(KINTER.EQ.6)THEN
          IF(IPR.eq.1)WRITE(36,62)J,SNU(1,J),SNU(2,J)       ! HISTAGT
...
C===================== end HISTAGT ========================
C
        IF(IPR.eq.1)WRITE(36,70)J,SNU(5,J),SNU(6,J),SNU(7,J),EKIN, ! HISTAGT
...
C
  ...
  123 format(/' MAKEGRID message: List of fragments after correction:')
      DO J=1,22
        write(34,124)J+8,INT(BOOKFR(1,J)),INT(BOOKFR(2,J))
