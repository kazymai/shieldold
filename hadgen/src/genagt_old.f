...
C --------------- Cleaning of INOVER and arrays SPT, SNU
        INOVER=0
      DO 1 J=1,5000        ! Changed be Timofeev 15/7/2011
      DO 1 I=1,6           ! --//--
    1 SPT(I,J)=0.
        DO 2 J=1,101       ! --//--
        DO 2 I=1,10        ! --//--
...
c      WRITE(25,215)R0N1,ANUCL1,A1,C1,D1,TF01,RM1
c      WRITE(25,215)R0N2,ANUCL2,A2,C2,D2,TF02,RM2
c  215 FORMAT(7F8.5)
cC ================end debug=======================
C
C * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
...
C      IF(INOVER.eq.1)WRITE(*,12312)
C12312 FORMAT('CASCAN FAILURE AT POINT 12312')
...
C   CHECK OF ARRAY SPT OVERFILLING or Cascan problems (IRET.ne.0)
      IF(MV.GE.LS100-1)THEN
         INOVER=1
         WRITE(25,1000)MV,IXINIT,NUMTRE
...
C Check of IRET           ! 13.02.96
      IF(IRET.NE.0)THEN
         INOVER=1
         WRITE(25,1001)IRET,IXINIT,NUMTRE
...
C ====Debug printout of Amelin's PPTCL and complete STAR (21.02.96)====
      IF(LSTAR.eq.2)WRITE(36,300)NUMTRE
...
C      ANUCL2=DBLE(NINT(ATWEI(NUCLID)))
      ...
      WRITE(*,11221)ANUCL2, ZNUCL2
11221 FORMAT('AMIXIS RETURNS: A=', F12.3, ' Z=', F12.3)
C========================== KarSjh ===================================
...
C Incident particle ----------------------
   ...
   12 RETURN
C
 1000 WRITE(25,1001) JPART,IXINIT,NUMTRE
...
C Is the particle No.J hadron or coalesced lightest nucleus?
      ...
      ELSE
          ...
          IF(IBAR.eq.4 .and. ICHR.eq.2)IPART=24
              IF(IPART.eq.0)WRITE(25,1001)NUMTRE
...
C         AGT's particle is converted
        write(25,11)PNAME(NN),PARTIC(JPART),TKMeV
   11     format(' IDP-->JPART:       ',A8,' IS CONVERTED TO ',A7,
     *           ', E(MeV)=',F10.1)
        return
      else
C         AGT's particle is ignored
        write(25,21)PNAME(NN),IDP,TKMeV
...
C
      IF(KFINIS.GE.LS100-1)THEN
         INOVER=1
         WRITE(25,1000)KFINIS,IXINIT,NUMTRE
...
C
      IF(LSTAR.eq.2)THEN
      IF(NPREC.gt.0)WRITE(36,1)
    1 FORMAT(' MEMAGT after PRECO')
C
      DO J=KSMEM1,KSMEMO-1
      WRITE(36,2)J,PMEMO(4,J)*1000.,PMEMO(5,J)*1000.,PMEMO(6,J)*1000.,
...
C
      ...
      IF(NPREC.gt.0)WRITE(36,3)
    3 FORMAT(1X,49('-'))
      IF(NPREC.gt.0)WRITE(36,4)PX1,PY1,PZ1,TNUCL1,APRECO,ZPRECO,UPRECO
...
C
   60 JPART=12   ! Gamma
      RETURN
C
 1000 WRITE(25,1001)ICHARG,RMASS,IXINIT,NUMTRE
...
C Particles
      DO J=1,MV
       ...
       WRITE(36,20)J,PMEMO(4,J)*1000.,PMEMO(5,J)*1000.,PMEMO(6,J)*1000.,
...
C
      WRITE(36,30)
   30 FORMAT(1X,49('-'))
      WRITE(36,40)PXSUM,PYSUM,PZSUM,ESUM
...
C Residue of target nucleus
      ...
      WRITE(36,60)PX2,PY2,PZ2,TNUCL2,AN2,ZN2,ENEXT2*1000.
...
C Residue of projectile nucleus
      ...
      PZ1=SNGL(PNUCL1(3)*1000.)
        A1=SNGL(AN1)
        TNUCL1=SQRT(PX1**2+PY1**2+PZ1**2+(940.*A1)**2)-(940.*A1)
      IF(JPART.ge.21)WRITE(36,70)PX1,PY1,PZ1,TNUCL1,AN1,ZN1,ENEXT1*1000.!HION2
...
C ------------It is particle
            WRITE(36,10)J,PX,PY,PZ,TKIN,AA,IZ,0,IA
   10         FORMAT(I3,1X,3E11.4,E13.4,F11.1,3X,3I2)
        ELSE
C ------------It is fragment
            WRITE(36,12)PX,PY,PZ,TKIN,FLOAT(IA),FLOAT(IZ),RESU
...
C
      DO J=1,NPTCL
      WRITE(36,2)J,pptcl(1,J)*1000.,pptcl(2,J)*1000.,pptcl(3,J)*1000.,
...
C         Conservation laws
        ...
        END IF
C
        WRITE(36,30)J,PX,PY,PZ,E,PARTIC(IPART)
...
C
  200 CONTINUE
      WRITE(36,39)
   39 FORMAT(66('-'))
      WRITE(36,40)PXSUM,PYSUM,PZSUM,ESUM,RMSUM,
...
C Is projectile Particle or Nucleus?
      ...
      WRITE(36,2) NAME(JPART),TINT,lanti
    2 FORMAT(5X,'INCIDENT PARTICLE IS ',A8/
     *       5X,'T0=',F11.1,' MEV.','  ANTILAB=',I1)
        ELSE
            IAPROJ=IFIX(APROJ)
            IZPROJ=IFIX(ZPROJ)
      WRITE(36,52) SYMB(IZPROJ),IZPROJ,IAPROJ,TINT
   52 FORMAT(5X,'INCIDENT HEAVY ION IS ',A2,'(',I2,',',I3,').'/
     *       5X,'T0=',F11.1,' MEV/A.')
        END IF
C
C Nucleus-target:      
      WRITE(36,3) SYMB(NUCLID),IFIX(ZNUC(NUCLID)),ATWEI(NUCLID)
...
c      WRITE(25,20)
   20 FORMAT(' No.       X1       Y1       Z1')
      ...
      DO J=1,INT(ANUCL1)
        ...
        Y1av=Y1av+YC(1,J)
        Z1av=Z1av+ZC(1,J)
c        WRITE(25,100)J,XC(1,J),YC(1,J),ZC(1,J)
  100   FORMAT(I4,3F9.4)
      END DO
c      WRITE(25,21)X1av,Y1av,Z1av
   21 FORMAT(' Rav',3F9.4)
C
c      WRITE(25,30)
   30 FORMAT(' No.       X2       Y2       Z2')
      ...
      DO J=1,INT(ANUCL2)
        ...
        Y2av=Y2av+YC(2,J)
        Z2av=Z2av+ZC(2,J)
c        WRITE(25,100)J,XC(2,J),YC(2,J),ZC(2,J)
      END DO
c      WRITE(25,21)X2av,Y2av,Z2av
C
      RIMPACT=SQRT((X1av-X2av)**2+(Y1av-Y2av)**2)
c      WRITE(25,40)RIMPACT
