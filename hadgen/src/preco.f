...
c      WRITE(25,237)U,A,Z,PNX,PNY,PNZ
...
C Check of PRECO Input --------
      IF(A.LT.0. .OR. Z.LT.0. .OR. U.LT.0.)THEN
        WRITE(25,238)U,A,Z,PNX,PNY,PNZ
...
c      write(25,378) U,A,Z
c  378 format(' U,A,Z:  ',F7.2,2F6.1)
c      write(25,379) PMEMO(8,K),PMEMO(9,K),
...
C
    5     IF(T-0.1)10,10,7
   10     IF(J-1)11,11,6
   11 TKINM=-DJ+SQRT(DJ**2+(RAN(IX)*(R2**2+2.*DJ*R2)))
      RETURN
C
    6 B1=RAN(IX)
