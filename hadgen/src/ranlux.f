      ...
      SUBROUTINE INIT_LUX(IXFIRS,LUXCNT,NUNIT)
C Initialization of the RANLUX(RVEC,LENV) random number generator
C from 32-bit integer IXFIRS as well as from the array ISVEC, if needs.
C If LUXCNT.eq.1, then input of the array ISVEC(25) is required for
C                    CALL RLUXIN(ISVEC).
C NUNIT is the number of output channel.
C-----------------------------------------------------------------
      DIMENSION ISVEC(25)
C
C  Set the Luxary level LUX, INT, K1, K2.
      LUX=4
      INT=IXFIRS
      K1=0
      K2=0
      ...
      WRITE(NUNIT,10)
   10 FORMAT(/5X,'Initial state of the RANLUX random generator.')
      WRITE(NUNIT,11)LUX,INT,K1,K2
   11 FORMAT(5X,'LUX,INT,K1,K2:',4I12)
C
C Input of the ISVEC(25), if required.
      IF(LUXCNT.eq.1)THEN
        OPEN(15,FILE='ISVEC.DAT', STATUS='OLD')
        READ(15,1)ISVEC
    1   FORMAT(5I14/5I14/5I14/5I14/5I14)
        CLOSE(15)
          CALL RLUXIN(ISVEC)
        WRITE(NUNIT,12)
   12   FORMAT(5X,'ISVEC(25):')
        WRITE(NUNIT,1)ISVEC
      END IF
C
      RETURN
      ...
      SUBROUTINE OUT_LUX(NUNIT)
C Output of the final state of the RANLUX(RVEC,LENV) random number
C generator in Unit No. NUNIT.
C-----------------------------------------------------------------
      DIMENSION ISVEC(25)
C
C  Get the Luxary level LUX and final values of INT, K1, K2.
      CALL RLUXAT(LUX,INT,K1,K2)
      WRITE(NUNIT,10)
   10 FORMAT(/5X,'Final state of the RANLUX random generator.')
      WRITE(NUNIT,11)LUX,INT,K1,K2
   11 FORMAT(5X,'LUX,INT,K1,K2:',4I12)
C
C Output of the final state of the array ISVEC(25).
      CALL RLUXUT(ISVEC)
      WRITE(NUNIT,12)
   12 FORMAT(5X,'ISVEC(25):')
      WRITE(NUNIT,1)ISVEC
...
*CMZ :          17/10/94  17.12.01  by  Unknown
*-- Author :
