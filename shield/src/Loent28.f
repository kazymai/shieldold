...
C
    ...
    2     WN(K)=SOURN(K,J)
      CALL LOENT(LABNOR)                                             ! GEMCA
          IF(LABNOR.NE.0)THEN
              WRITE(25,1001)LABNOR,IXINIT,NUMTRE
...
c                 COMMENTED BY A.TIMOFEEV
                 WRITE(25,1001)SYMB(NUCLID),IFIX(ZNUC(NUCLID)),
     *                                      NINT(ATWEI(NUCLID))
...
C                 STOP
                  IF(OLN.LT.14.5)THEN                         ! BNAB28
                      STOP
                  ELSE
C                      WRITE(27,1001)SYMB(NUCLID),IFIX(ZNUC(NUCLID)),
C     *                                           NINT(ATWEI(NUCLID))
                  END IF
C------------------------------------------------------------------
              ELSE
...
C      DO J=-1,26                                               ! BNAB28
C        WRITE(27,30)J,(SIMALN(J,1,K),K=1,12)
...
C      DO J=-1,26                                               ! BNAB28
C        WRITE(27,30)J,(SIMALN(J,1,K),K=13,24)
C      END DO
C                   END IF
C
...
C      DO J=-1,26                                               ! BNAB28
C        WRITE(27,30)J,(SIMALN(J,1,K),K=25,36)
C      END DO
C                   END IF
C
...
C      DO J=-1,26                                               ! BNAB28
C        WRITE(27,30)J,(SIMALN(J,1,K),K=37,48)
C      END DO
C                   END IF
C
c      WRITE(27,25)
...
c   26 WRITE(27,24)J,(SIMALN(J,2,K),K=1,16)
C ---------------------- end CORNEL ------------------------
C
      RETURN
      END
...
C
   ...
   30 FORMAT(' GR.',5X,'ENERGY (MEV)',8X,'S-TOT',6X,'S-FIS',5X,
     *       'AVER NU',5X,'S-CAP',6X,'S-IN',7X,'S-EL',6X,'COS EL')
      DO 40 J=-1,25                                               ! BNAB28
   40 WRITE(27,50)J,EGR(J+1),EGR(J),SGLN(J,7,IRN),(SGLN(J,K,IRN),K=1,6)
   50 FORMAT(1X,I2,2X,E9.3,'-',E9.3,7(2X,F9.4))
      WRITE(27,60)SGLN(26,7,IRN),(SGLN(26,K,IRN),K=1,6)
...
C
      DO 90 J=-1,12                                               ! BNAB28
   90 WRITE(27,100)J,(SUV(K,J,IRN),K=1,13)
...
C DETERMINATION OF THE NEUTRON STATUS IN THE END POINT OF RANGE
      ...
      WRITE(25,1001)LNSTAT,IXINIT,NUMTRE,WN0
...
c      EEE=WN(7)
c      TTT=WN(11)
c      WRITE(25,3456)NUMTRE,RRR,EEE,TTT
...
C CHECK OF NEUTRON MEMORY OVERFILLING
      IF(NREMLN+NFISN .GT. LRL50)THEN
          WRITE(25,1002)NFISN,IXINIT,NUMTRE
...
C SAMPLING OF SECONDARY NEUTRONS NUMBER  NSECN
      SIGN2N=SUV(13,NGRUP,IRENLN)-SGLN(NGRUP,4,IRENLN)
          IF(SIGN2N.LE.0.)THEN
              ...
              GO TO 64
          ELSE
              IF(RAN(IX).LE.(SIGN2N/SGLN(NGRUP,4,IRENLN)))THEN
                  NSECN=2
                  GO TO 64
              ELSE
                  NSECN=1
...
C CHECK OF NEUTRON MEMORY OVERFILLING
   64 IF(NREMLN+NSECN .GT. LRL50)THEN
          WRITE(25,1003)NSECN,IXINIT,NUMTRE
...
c          write(25,312)NUMTRE,WN(7),REMLN(7,NREMLN+1),REMLN(7,NREMLN+2),
...
C     CHECK OF STEPLN OVERFILLING
      IF(ISTEP.GT.LNN50)THEN
          WRITE(25,1002)ISTEP,NUMTRE,IXINIT
...
C                      WN(11)=TIME                                   ! GEMCA
              ...
              RETURN
          ELSE
              WRITE(25,1004)KSTATE,MEDCUR,NUMTRE,IXINIT              ! GEMCA
...
C
   15 GAMMA=RAN(IX)                         ! Sobol, 15.04.99
...
C
      IF(NGRUP.GT.12)THEN
          WRITE(25,1001)NGRUP,IXINIT,NUMTRE
...
C
 ...
 1001 FORMAT(' TNEUT DIAGNOSTIC: ILLEGAL IPR=',I3)
      STOP
C
   10 BETHA=RAN(IX)*SPEFIS(13)           ! (n,f)
      DO 11 J=-1,13                                               ! BNAB28
          IF(BETHA .LE. SPEFIS(J))THEN
              NGR=J
              TNEUT=EGR(NGR+1)+(EGR(NGR)-EGR(NGR+1))*RAN(IX)
              GO TO 30
          ELSE
              GO TO 11
          END IF
   11 CONTINUE
C
   20 BETHA=RAN(IX)*SPECIN(12)           ! (n,n')
      DO 21 J=1,12
        IF(BETHA.LE.SPECIN(J))THEN
          NGR=NGRUP+J-1
              if(J.eq.1)then    ! correction of 10.04.97
                TNEUT=EGR(NGR+1)+(WN(7)   -EGR(NGR+1))*RAN(IX)
              else
                TNEUT=EGR(NGR+1)+(EGR(NGR)-EGR(NGR+1))*RAN(IX)
              end if
          GO TO 30
...
C    Cos(chi/2)=Cos(theta)=SQRT(gamma)
   20 COST=SQRT(RAN(IX))     ! Cos(theta)=SQRT(gamma)
      CALL UNIFI(SINF,COSF)
...
C CHECK OF NUMBER OF ZONES
      IF(LNZON.GT.LES50)THEN
          WRITE(25,1000)LNZON,IXINIT,NUMTRE
...
C REGISTRATION OF INELASTICU INTERACTIONS (N,N'),(N,2N)
   60 CONTINUE
      IF(NSECN.LT.1 .OR. NSECN.GT.2)THEN
          WRITE(25,1002)NSECN,IXINIT,NUMTRE
...
C
      DO 30 J=-1,26                                               ! BNAB28
   30 WRITE(24,40)J,SOURLN(J),SPLOUT(J),RESTRE(J)
   40 FORMAT(I5,3F12.3)
      WRITE(24,50)SOURLN(27),SPLOUT(27),RESTRE(27)
   50 FORMAT('    N',3F12.2)
      WRITE(24,60)SOURLN(28),SPLOUT(28),RESTRE(28)
