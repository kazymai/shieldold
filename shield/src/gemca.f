...
C
  100   CONTINUE
        CALL GBWRT(JOUT,ISOB,RTMP,JTMP) ! Sobol: ISOB instead of I
...
C
	IF(ITYPE.EQ.IMOV)THEN
	   CALL GBMOV(JREAD,NBT,RTMP,JTMP)
	   ISOB=NBT                       ! Sobol: ISOB instead of I
	   GOTO 102
	ELSEIF(ITYPE.EQ.IROT)THEN
	   CALL GBROT(JREAD,NBT,RTMP,JTMP)
	   ISOB=NBT                       ! Sobol: ISOB instead of I
	   GOTO 102
	ENDIF
C
...
C
	...
	GOTO 100
C
200     CALL GBWRT(JOUT,-1,RTMP,JTMP)
C
...
C
	...
	IF(JDBG1.NE.0)THEN
	 CLOSE(JOUT,IOSTAT=IFSTA2,STATUS='DELETE',ERR=1000)
	ELSE
	 CLOSE(JOUT,IOSTAT=IFSTA2,STATUS='KEEP',ERR=1000)
	ENDIF
	IF(IFSTA1.NE.0 .OR. IFSTA2.NE.0)THEN
	 PRINT *,'GEOINI: Some file error was encountered'
	 PRINT *,'Input  file IOSTAT =',IFSTA1
	 PRINT *,'Output file IOSTAT =',IFSTA2
	 PRINT *,'                    '
	ENDIF
...
C
	...
	SUBROUTINE GBMOV(JREAD,NBT,RTMP,JTMP)
C****************************************************************
...
C
	...
	CALL GBREAD(JREAD,NB,RBDATA,JBDATA)
	CALL GFEND(JREAD)
C
	X=RTMP(2)
	Y=RTMP(3)
...
C
	...
	SUBROUTINE GBROT(JREAD,NBT,RTMP,JTMP)
C****************************************************************
...
C
	...
	SUBROUTINE GFEND(LUN)
C****************************************************************
C*  Jump to the end of file                                     *
C****************************************************************
1       FORMAT(2X,A3)
2       READ(LUN,1,END=3)ITMP
	GOTO 2
3       RETURN
	END
	...
	SUBROUTINE GBREAD(JREAD,NB,RBDATA,JBDATA)
C****************************************************************
...
100     IF(NB.EQ.NBCUR)THEN
	...
	ENDIF
C
	READ(JREAD,97) ITYPE,NBFIL,(RBDATA(I),I=1,6)
C
101       DO IBT=1,10
...
C
C----- BODY =  "WED" -----
2       READ(JREAD,98) (RBDATA(I),I=7,12)
C
	GOTO 100
C
C----- BODY =  "ARB" -----
3       READ(JREAD,98) (RBDATA(I),I=7,12)
	READ(JREAD,98) (RBDATA(I),I=13,18)
	READ(JREAD,98) (RBDATA(I),I=19,24)
	READ(JREAD,99) (JBDATA(I),I=1,6)
C
	GOTO 100
C
C----- BODY =  "BOX" -----
4       READ(JREAD,98) (RBDATA(I),I=7,12)
C
	GOTO 100
C
C----- BODY =  "RPP" -----
...
C
C----- BODY =  "RCC" -----
6       READ(JREAD,98) RBDATA(7)
C
	GOTO 100
C
C----- BODY =  "REC" -----
7       READ(JREAD,98) (RBDATA(I),I=7,12)
C
	GOTO 100
C
C----- BODY =  "TRC" -----
8       READ(JREAD,98) (RBDATA(I),I=7,8)
C
	GOTO 100
C
C----- BODY =  "ELL" -----
9       READ(JREAD,98) (RBDATA(I),I=7,12)
C
	GOTO 100
C
C----- END OF BODY DATA ---------------
C
	...
	SUBROUTINE GBWRT(JOUT,IBT,RTMP,JTMP)
C****************************************************************
...
C
	...
	JDBG1=0
	JDBG2=0
	WRITE(JOUT,21)JDBG1,JDBG2,(JTMP(I),I=1,15)
...
99      FORMAT(10X,6(I9,1X))
C
	WRITE(JOUT,97) ISYMB(IBT),NUMBOD,(RTMP(I),I=1,6)
C
...
C
C----- BODY =  "WED" -----
2       WRITE(JOUT,98) (RTMP(I),I=7,12)
C
	RETURN
C
C----- BODY =  "ARB" -----
3       WRITE(JOUT,98) (RTMP(I),I=7,12)
	WRITE(JOUT,98) (RTMP(I),I=13,18)
	WRITE(JOUT,98) (RTMP(I),I=19,24)
	WRITE(JOUT,99) (JTMP(I),I=1,6)
C
	RETURN
C
C----- BODY =  "BOX" -----
4       WRITE(JOUT,98) (RTMP(I),I=7,12)
C
	RETURN
C
C----- BODY =  "RPP" -----
...
C
C----- BODY =  "RCC" -----
6       WRITE(JOUT,98) RTMP(7)
C
	RETURN
C
C----- BODY =  "REC" -----
7       WRITE(JOUT,98) (RTMP(I),I=7,12)
C
	RETURN
C
C----- BODY =  "TRC" -----
8       WRITE(JOUT,98) (RTMP(I),I=7,8)
C
	RETURN
C
C----- BODY =  "ELL" -----
9       WRITE(JOUT,98) (RTMP(I),I=7,12)
C
	RETURN
C
C----- END OF BODY DATA ---------------
C
	...
	SUBROUTINE GZWRT(JOUT,NNN,IALP,NAZ,IZSYM,JTMP,JTY1,JTY2)
C****************************************************************
...
2       FORMAT(2X,A3,5X,9(A2,I5))
	IF(IALP.EQ.IBL)THEN
	 WRITE(JOUT,1)IALP,(IZSYM(I),JTMP(I),I=1,9)
	ELSE
	 WRITE(JOUT,2)IALP,NAZ,(IZSYM(I),JTMP(I),I=1,9)
	ENDIF
C
	ELSE
C
3       FORMAT(14I5)
	DO I=1,L
	 WRITE(JOUT,3)(JTY1(J+(I-1)*14),J=1,14)
	END DO
C
	DO I=1,L
	 WRITE(JOUT,3)(JTY2(J+(I-1)*14),J=1,14)
	END DO
C
	ENDIF
C
